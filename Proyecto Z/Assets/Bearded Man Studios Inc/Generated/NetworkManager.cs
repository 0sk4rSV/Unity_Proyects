...
namespace BeardedManStudios.Forge.Networking.Unity
{
	public partial class NetworkManager : MonoBehaviour
	{
		...
		public GameObject[] R_DesconectaNetworkObject = null;
		public GameObject[] R_ObjetoGenericoNetworkObject = null;
		public GameObject[] R_PlayerNetworkObject = null;
		public GameObject[] R_PowerUpNetworkObject = null;
		public GameObject[] R_ZombieNetworkObject = null;
		...
		private void CaptureObjects(NetworkObject obj)
		{
			...
			else if (obj is R_DesconectaNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (R_DesconectaNetworkObject.Length > 0 && R_DesconectaNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(R_DesconectaNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<R_DesconectaBehavior>();
						}
					}
			...
			else if (obj is R_ObjetoGenericoNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (R_ObjetoGenericoNetworkObject.Length > 0 && R_ObjetoGenericoNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(R_ObjetoGenericoNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<R_ObjetoGenericoBehavior>();
						}
					}
			...
			else if (obj is R_PlayerNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (R_PlayerNetworkObject.Length > 0 && R_PlayerNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(R_PlayerNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<R_PlayerBehavior>();
						}
					}
			...
			else if (obj is R_PowerUpNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (R_PowerUpNetworkObject.Length > 0 && R_PowerUpNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(R_PowerUpNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<R_PowerUpBehavior>();
						}
					}
			...
			else if (obj is R_ZombieNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (R_ZombieNetworkObject.Length > 0 && R_ZombieNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(R_ZombieNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<R_ZombieBehavior>();
						}
					}
		...
		[Obsolete("Use InstantiateR_Desconecta instead, its shorter and easier to type out ;)")]
		public R_DesconectaBehavior InstantiateR_DesconectaNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(R_DesconectaNetworkObject[index]);
			var netBehavior = go.GetComponent<R_DesconectaBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<R_DesconectaBehavior>().networkObject = (R_DesconectaNetworkObject)obj;

			FinalizeInitialization(go, netBehavior, obj, position, rotation, sendTransform);
			
			return netBehavior;
		}
		[Obsolete("Use InstantiateR_ObjetoGenerico instead, its shorter and easier to type out ;)")]
		public R_ObjetoGenericoBehavior InstantiateR_ObjetoGenericoNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(R_ObjetoGenericoNetworkObject[index]);
			var netBehavior = go.GetComponent<R_ObjetoGenericoBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<R_ObjetoGenericoBehavior>().networkObject = (R_ObjetoGenericoNetworkObject)obj;

			FinalizeInitialization(go, netBehavior, obj, position, rotation, sendTransform);
			
			return netBehavior;
		}
		[Obsolete("Use InstantiateR_Player instead, its shorter and easier to type out ;)")]
		public R_PlayerBehavior InstantiateR_PlayerNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(R_PlayerNetworkObject[index]);
			var netBehavior = go.GetComponent<R_PlayerBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<R_PlayerBehavior>().networkObject = (R_PlayerNetworkObject)obj;

			FinalizeInitialization(go, netBehavior, obj, position, rotation, sendTransform);
			
			return netBehavior;
		}
		[Obsolete("Use InstantiateR_PowerUp instead, its shorter and easier to type out ;)")]
		public R_PowerUpBehavior InstantiateR_PowerUpNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(R_PowerUpNetworkObject[index]);
			var netBehavior = go.GetComponent<R_PowerUpBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<R_PowerUpBehavior>().networkObject = (R_PowerUpNetworkObject)obj;

			FinalizeInitialization(go, netBehavior, obj, position, rotation, sendTransform);
			
			return netBehavior;
		}
		[Obsolete("Use InstantiateR_Zombie instead, its shorter and easier to type out ;)")]
		public R_ZombieBehavior InstantiateR_ZombieNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(R_ZombieNetworkObject[index]);
			var netBehavior = go.GetComponent<R_ZombieBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<R_ZombieBehavior>().networkObject = (R_ZombieNetworkObject)obj;
		...
		/// <summary>
		/// Instantiate an instance of R_Desconecta
		/// </summary>
		/// <returns>
		/// A local instance of R_DesconectaBehavior
		/// </returns>
		/// <param name="index">The index of the R_Desconecta prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public R_DesconectaBehavior InstantiateR_Desconecta(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(R_DesconectaNetworkObject[index]);
			var netBehavior = go.GetComponent<R_DesconectaBehavior>();
		...
		/// <summary>
		/// Instantiate an instance of R_ObjetoGenerico
		/// </summary>
		/// <returns>
		/// A local instance of R_ObjetoGenericoBehavior
		/// </returns>
		/// <param name="index">The index of the R_ObjetoGenerico prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public R_ObjetoGenericoBehavior InstantiateR_ObjetoGenerico(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(R_ObjetoGenericoNetworkObject[index]);
			var netBehavior = go.GetComponent<R_ObjetoGenericoBehavior>();
		...
		/// <summary>
		/// Instantiate an instance of R_Player
		/// </summary>
		/// <returns>
		/// A local instance of R_PlayerBehavior
		/// </returns>
		/// <param name="index">The index of the R_Player prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public R_PlayerBehavior InstantiateR_Player(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(R_PlayerNetworkObject[index]);
			var netBehavior = go.GetComponent<R_PlayerBehavior>();
		...
		/// <summary>
		/// Instantiate an instance of R_PowerUp
		/// </summary>
		/// <returns>
		/// A local instance of R_PowerUpBehavior
		/// </returns>
		/// <param name="index">The index of the R_PowerUp prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public R_PowerUpBehavior InstantiateR_PowerUp(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(R_PowerUpNetworkObject[index]);
			var netBehavior = go.GetComponent<R_PowerUpBehavior>();
		...
		/// <summary>
		/// Instantiate an instance of R_Zombie
		/// </summary>
		/// <returns>
		/// A local instance of R_ZombieBehavior
		/// </returns>
		/// <param name="index">The index of the R_Zombie prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public R_ZombieBehavior InstantiateR_Zombie(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(R_ZombieNetworkObject[index]);
			var netBehavior = go.GetComponent<R_ZombieBehavior>();
